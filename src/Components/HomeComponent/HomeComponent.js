...
const HomeComponent = () => {
    ...
    const [ square, setSquare ] = useState( { geometry: null, material: null, scene: null, mesh: null } )
    ...
    useEffect(()=>{
      ...
      const threejs = () => {
		...
		const material = new THREE.MeshBasicMaterial( { color: '#ededed', wireframe: true } );
      ...
      const runFrame = ( scene, camera, renderer, threeObject ) => {
        ...
        requestAnimationFrame( () => { runFrame( scene, camera, renderer, threeObject ) } )
    ...
    return (
        <>
           <Grid container style={ { height: '100vh', justifyContent: 'flex-start', alignItems: 'center' } }>
                <Grid item xl={2} lg={2} md={3} sm={4} xs={12}>
                    <Paper elevation={0} style={ { padding: '10px' } }>
                        <Button onClick={ () => { navigateTo( 'skills' ) } } fullWidth menu-btn="true" >Skills</Button>
                        <Button onClick={ () => { navigateTo( 'projects' ) } } fullWidth menu-btn="true">Projects</Button>
                        <Button onClick={ () => { navigateTo( 'about' ) } } fullWidth menu-btn="true">About</Button>
                    </Paper>
                </Grid>
                <Grid item xl={4} lg={4} md={5} sm={6} xs={12} style={ { display: 'flex', zIndex: '1' } }>
